...
PlanTrace includes an evidence-grounded Query Regression Investigator for query detail views and API consumers.

## What it does

- Reads query fingerprints, metric trends, regression rows, and diagnostic findings
- Produces a structured report with summary, risk level, confidence, likely causes, evidence, and suggested actions
...
- `test_ai_investigation.py` covers fake-provider success, invalid JSON, timeout fallback, schema validation, grounding rejection, and the API endpoint
